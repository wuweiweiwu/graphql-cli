...
async function askForEnum<T, Enum extends StandardEnum<T>>(options: {
  enum: Enum;
  message: string;
  defaultValue?: T;
  ignoreList?: (T | string)[];
...
export const plugin: CliPlugin = {
  init({ program, reportError }) {
    program
      ...
      .option(
        '--templateUrl',
        'GitHub URL of the template. For example (http://github.com/example/graphql-cli-example-template)'
      )
      .action(
        async ({
          projectName,
          ...
          try {
            ...
            if (initializationType === InitializationType.FromScratch) {
              ...
              if (!templateName) {
                ...
                const templateNames = Object.keys(templateMap).filter(
                  (templateName) => templateMap[templateName].projectType === projectType
                );
            ...
            if (
              !graphqlConfig.documents &&
              (projectType === ProjectType.FullStack || projectType === ProjectType.FrontendOnly)
            ) {
